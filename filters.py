...
def filter_signal(
    signal, order=8, sample_rate=20000, freq_low=400, freq_high=2000, axis=0
):
...
if __name__ == "__main__":
    ...
    path = r"D:\F.LARENO.FACCINI\Preliminary Results\Ephy\Coordinate Hunting\854\RBF\CrusI\P19\2019-10-01T15-01-48_1500um_P19_crusI.rbf"
    ...
    duration = 1.0 / sampling_rate * len(data[:, 0])
    sig_times = np.arange(0, duration, 1.0 / sampling_rate)

    notch = notch_filter(signal=data[:, 1])

    plt.plot(
        sig_times, data[:, 1], linewidth=0.3, alpha=0.3, color="k", label="Raw signal"
    )
    plt.plot(
        sig_times,
        filter_signal(notch, freq_low=freq_low, freq_high=freq_high),
        linewidth=0.3,
        color="b",
